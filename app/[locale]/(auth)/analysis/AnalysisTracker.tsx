...
} from "@/app/[locale]/(auth)/analysis/analysisStatus";
...
const AnalysisTracker = () => {
    const t = useTranslations("analysis.AnalysisTracker");
    ...
    useEffect(() => {
        ...
        const eventSource = new EventSource(`${mockAnalysisEventApiPath}`);
    ...
    return (<>
            {
                isAnalysisPending ?
                    ...
                    :
                    <Button onClick={handleTriggerAnalysis} size={"fullW"}>{t("run")}</Button>
            }
        </>
    );
};
